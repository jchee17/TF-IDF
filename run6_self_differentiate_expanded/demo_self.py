...
self_diff = np.load("./data_objects/self_diff.npy")
self_diff_stopwords = np.load("./data_objects/self_diff_stopwords.npy")
...
def relevant_concepts(doc_comparison, tfidf_articles, tfidf_concepts, list_articles, list_concepts, list_common_vocab, article_idx, k):
    ...
    if article_idx in range(len(list_articles)):
        ...
        for idx in sorted_idx:
            print("\t Concept: {}, score: {}".format(list_wiki[idx].decode('utf-8'), round(scores[idx], 5)))
            ...
            for idx2 in sorted_idx2:
                print("\t\t word = {}, score = {}".format(list_common_vocab[idx2], norm_prod[idx2]))
    else:
        print("\tError: input doc not in given dictionary")

def relevant_concepts_reverse(doc_comparison, tfidf_articles, tfidf_concepts, list_articles, list_concepts, list_common_vocab, article_idx, k):
    print("Top concepts in document {}:".format(article_idx))
    ...
    if article_idx in range(len(list_articles)):
        ...
        for idx in sorted_idx:
            print("\t[{}] Concept: {}, score: {}".format(idx, list_wiki[idx].decode('utf-8'), 
                round(scores[idx], 5)))
            article_vec = tfidf_articles[article_idx]
            concept_vec = tfidf_concepts[idx]
            norm_prod   = np.divide(np.multiply(article_vec, concept_vec), LA.norm(article_vec)*LA.norm(concept_vec))
...
def show_doc_inbrowser(article_str):
    ...
    webbrowser.open('https://en.wikipedia.org/wiki/' + id_string)
